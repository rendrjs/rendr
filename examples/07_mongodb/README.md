...
The purpose of this little app is to demonstrate one way of using Rendr to build a web app that runs on both the client and the server. This particular example demonstrates a Rendr app that receives its data from a local [MongoDB](http://www.mongodb.org/) instance rather than directly from GitHub.
...
Secondly, make sure that you have MongoDB [installed on your system](http://docs.mongodb.org/manual/installation/).

Run `npm install` to install dependencies:

    $ npm install

Run `grunt prepare-db` to initialize a local MongoDB instance:
